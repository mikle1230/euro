...
## 2026-09-18 — 右侧面板默认收起 + 空态指路（刀4）

**来源**：Michael「刀4 也可以做完」；A 方案最后一刀（地图当主角的收口）。
**状态**：🛠️ 已实现

### 改了什么
- `src/app/explore/page.js`：`panelCollapsed` 初值 `false` → **`true`**（桌面端）。打开 `/explore` 先看到的就是一张线；面板只留右侧一条 `◀` 拉条。
- 新增**空态卡片**：仅在 `!activeItinerary` 时出现 —— 因为面板默认收起后，「导入」按钮（住在面板里，header 没有）不再默认可见，必须有明确指路。按钮「展开面板导入」= `setPanelCollapsed(false)`。有行程时该卡片完全不存在。

### 为什么这么做
- 刀3 把条目降为抽屉后，面板的默认展开就变成“挡住地图的默认状态”——与 A 方案验收标准（**打开不点任何按钮就能说出几天/多少公里/哪天最赶/哪天走回头路**）直接冲突。
- **移动端不受影响**：`showMap = !isMobile`，移动端不渲染地图、面板仍铺满（此值对移动端不生效）。
- 「全部条目 >」仍会**展开面板并保持展开**（`handleOpenFullPanel` 已同时 `setPanelCollapsed(false)`），深度模式入口不变。

### 实测（真机 Chromium headless，12 项断言全过）
- 有行程：面板**默认收起**（`aria-label="展开面板"` 存在）；行程条 3 天渲染；点 chip 开抽屉（`aria-hidden=false`）；**Esc 关**（`aria-hidden=true`）；条目默认折叠→可展开；无页面错误。
- 无行程：面板默认收起 + **空态提示出现**；点「展开面板导入」→ 面板展开（`aria-label="收起面板"` 出现）且能看到「导入」；无页面错误。
- 注：抽屉是**常驻 DOM**（靠 `translateX(-100%)` 移出），判断开合要读 **`aria-hidden`**，不能数元素个数 —— 第一版断言就是这么写错、误报了一个 FAIL。

### 遗留 / 风险
- **导入后不自动收起面板**：导入完成后面板仍展开并落到「行程详情」（既有行为未动）。若想让“导入即看到线”，需要新增“导入成功后自动收起”——**待 Michael 确认**。
- **窄视口**：抽屉（左 380）与面板（右 ≥360）同开时在 <800px 会贴近；层级已差开（1050 / 1000），但未做联动。
- 未改：导入流程、header、AI 解析链、`src/data/*`。
